...
public class ItemEventFrame extends EventFrame {
	...
	private void preparar() {
		...
		for (int i = 0; i < NUM_CHKBOXES; i++) {
			...
			adicionaComponente(chkBoxes[i], 0, i, 1, 1);
		}
		...
		for (int i = 0; i < NUM_RADIOBTNS; i++) {
			...
			adicionaComponente(radioBtns[i], 1, i, 1, 1);
		}

		comboBox = new JComboBox<String>(LIST_ITEMS);
		comboBox.setMaximumRowCount(3);
		comboBox.addItemListener(new ItemEventHandler("ComboBox"));
		adicionaComponente(comboBox, 2, 0, Math.max(NUM_CHKBOXES, NUM_RADIOBTNS), 1);
		...
		adicionaComponente(btn, 3, 0, Math.max(NUM_CHKBOXES, NUM_RADIOBTNS), 1);
	}
